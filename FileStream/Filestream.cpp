...
{
	string m_name = "";//스트링변수를생성한다. m_name이라고 이름짓는다. 기본값은 ""이다.
	int m_math = 0;
	int m_eng = 0;
	int m_lang = 0;
...
//& = 레퍼런스 참조 , 원본을 보여주는것
void pushStTest(vector<stTest>& vectest, string& str)//보이드함수를 만든다.반환값은없다. 이름은 pushStTest ()안에 변수를 넣는다.
                                                     //vector<stTest>& 참조변수생성하고 이름은 vectest, 
...
int main()
{
	...
	if (readFile.is_open())//만약 readFile이 열려있다면 실행한다.
	{
		while (!readFile.eof())//읽는 (eof=)포인트가 더이상 없으면 false
		{
			...
			pushStTest(vectest, str);//pushStTest함수에 str을 집어넣는다.
        }
